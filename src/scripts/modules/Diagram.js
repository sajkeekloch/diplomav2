import { numberOfWeek, dayOfWeek } from '../constants/constants';
...
class Diagram {
    ...
    showDates(dates, rays) {
      ...
      numberOfWeek.forEach((el) => {
        ...
        if (sessionStorage.getItem(`ray${el}`) < 2) {
          percent.style.display = 'none';
        } else {
          percent.textContent = sessionStorage.getItem(`ray${el}`);
        }
  
        if (!(sessionStorage.getItem(`ray${el}`))) {
          ray.style.width = 0;
        } else {
          ray.style.width = `${sessionStorage.getItem(`ray${el}`)}%`;
        }
      });
    }
  }
